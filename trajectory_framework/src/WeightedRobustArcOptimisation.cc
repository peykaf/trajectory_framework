...
WeightedRobustArcOptimisation::WeightedRobustArcOptimisation(json &json_input) : WeightedRobustOptimisation()
{
    std::cout << "Weighted robust arc optimisation!" << std::endl;
...
void WeightedRobustArcOptimisation::price_unused_photon_cpts()
{
    ...
    for (size_t cpt_index = 1; cpt_index < this->photon_cpts.size(); cpt_index++)
    {
        ...
        if (!cur_cpt->included)
        {
            ...
            cur_cpt->price_constrained_aperture(this->lag_mults, this->scenario_weights, this->structure_sets, prev_cpt, next_cpt);
        }
    }
...
size_t WeightedRobustArcOptimisation::count_electron_apertures()
{
...
bool WeightedRobustArcOptimisation::add_new_weight()
{
...
void WeightedRobustArcOptimisation::export_progress()
{
