...
RobustOptimisation::RobustOptimisation() {
...
RobustOptimisation::RobustOptimisation(json &json_input) : RobustOptimisation()
{
    std::cout << "Robust optimisation!" << std::endl;
    this->input_filename = json_input["name"];

    this->from_json(json_input);
}

void RobustOptimisation::from_json(json &json_input)
{
...
void RobustOptimisation::get_starting_point(double *weights)
{
    double max_cost = 0.0;
    for (size_t ap_index = 0; ap_index < this->aperture_sets.size(); ap_index++)
    {
        ...
        for (auto &structure : this->structure_sets[ap_index])
        {
            current_cost += structure->calculate_cost();
        }
    ...
    {
        weights[i + 1] = this->aperture_sets[0][i]->weight;
    }
}

void RobustOptimisation::update_weights(const double *weights)
{
    for (auto &apertures : this->aperture_sets)
    {
        for (size_t i = 0; i < apertures.size(); i++)
        {
            // weights[0] is a dummy variable, weights[1] - weights[N] are the decision variables.
            apertures[i]->set_weight(weights[i + 1]);
        }
    }
}

void RobustOptimisation::update_weights(const double *weights, std::vector<WeightClass *> apertures)
{
    for (size_t i = 0; i < apertures.size(); i++)
    {
        apertures[i]->set_weight(weights[i + 1]);
...
void RobustOptimisation::calculate_structure_dose(Structure *structure, std::vector<WeightClass *> apertures)
{
...
void RobustOptimisation::update_doses()
{
    for (size_t ap_index = 0; ap_index < this->aperture_sets.size(); ap_index++)
    {
        for (auto &structure : this->structure_sets[ap_index])
...
void RobustOptimisation::update_doses(const double *weights)
{
...
void RobustOptimisation::update_kkt_mults(const double *lambdas)
{
    for (size_t i = 0; i < this->num_scenarios; i++)
    {
        kkt_mults[i] = lambdas[i];
    }
}

void RobustOptimisation::calculate_constraints(double *constraints, const double *weights, bool new_weights)
{
    this->latest_cost = weights[0];
    ...
    for (size_t ap_index = 0; ap_index < this->aperture_sets.size(); ap_index++)
    {
        ...
        constraints[ap_index] = current_cost - weights[0];
    }
}

void RobustOptimisation::calculate_jacobian(double *values, const double *weights, bool new_weights)
{
    if (new_weights)
    {
        this->update_doses(weights);
    ...
    // Constraints are all of the form t - f_i >= 0
    for (size_t ap_set_i = 0; ap_set_i < this->aperture_sets.size(); ap_set_i++)
    {
        // There are num_weights + 1 decision variables.
        size_t offset = ap_set_i * (this->num_weights + 1);
        values[offset] = -1.0;

        for (size_t ap_index = 0; ap_index < this->aperture_sets[ap_set_i].size(); ap_index++)
        {
            double grad_value = 0.0;
            for (auto &structure : this->structure_sets[ap_set_i])
            {
                grad_value += structure->calculate_gradient(this->aperture_sets[ap_set_i][ap_index]->dose);
            }
            values[offset + 1 + ap_index] = grad_value;
        }
    }
}

void RobustOptimisation::calculate_hessian(double *hess_values, const double *weights, bool new_weights, const double *lambdas, bool new_lambdas)
{
    if (new_weights)
    {
        this->update_doses(weights);
    ...
    for (size_t row = 0; row < this->num_weights; row++)
    {
        for (size_t col = 0; col <= row; col++)
        {
            ...
            for (size_t ap_set_i = 0; ap_set_i < this->aperture_sets.size(); ap_set_i++)
            {
                ...
                for (auto &structure : this->structure_sets[ap_set_i])
                {
                    sc_hess += structure->calculate_hessian(this->aperture_sets[ap_set_i][row]->dose,
                                                            this->aperture_sets[ap_set_i][col]->dose);
                }

                hess += lambdas[ap_set_i] * sc_hess;
            }
...
std::vector<double> RobustOptimisation::calculate_total_dose()
{
...
std::vector<double> RobustOptimisation::calculate_scenario_dose(size_t sc_i)
{
    std::vector<double> total_dose(this->total_voxels, 0.0);
...
int RobustOptimisation::get_num_apertures()
{
    return this->num_weights;
}

double RobustOptimisation::avg_aperture_size()
{
...
void RobustOptimisation::update_data()
{
...
void RobustOptimisation::output_plan_statistics()
{
...
bool RobustOptimisation::prune_cpts()
{
    ...
    {
        for (size_t set_index = 0; set_index < this->aperture_sets.size(); set_index++)
        {
            for (size_t i = 0; i < this->aperture_sets[set_index].size(); i++)
            {
                if (this->aperture_sets[set_index][i]->weight < avg_weight / 1000.0)
                {
                    this->aperture_sets[set_index][i]->deactivate();
                    this->pruned_apertures += 1;
                }
            }

            this->aperture_sets[set_index].erase(std::remove_if(this->aperture_sets[set_index].begin(), this->aperture_sets[set_index].end(),
                                                                [](WeightClass *ap) { return !(ap->active); }),
                                                 this->aperture_sets[set_index].end());
        }
...
void RobustOptimisation::prepare_pricing()
{
    ...
    // Export intermediate results every 10 iterations.
...
bool RobustOptimisation::add_new_aperture()
{
...
void RobustOptimisation::calculate_lag_mults()
{
    for (size_t ap_index = 0; ap_index < this->aperture_sets.size(); ap_index++)
    {
        std::fill(this->lag_mults[ap_index].begin(), this->lag_mults[ap_index].end(), 0.0);
        ...
        for (auto &structure : this->structure_sets[ap_index])
        {
            structure->calculate_lag_mults(this->lag_mults[ap_index]);
        }
    }
}

void RobustOptimisation::add_aperture(RobustControlPoint *cpt)
{
    ...
    for (size_t i = 0; i < this->aperture_sets.size(); i++)
    {
        this->aperture_sets[i].push_back(apertures[i]);
    }

    this->num_weights = this->aperture_sets[0].size();
}

bool RobustOptimisation::random_scheme()
{
...
bool RobustOptimisation::add_all_generated_apertures_scheme()
{
...
bool RobustOptimisation::best_each_particle_scheme()
{
...
bool RobustOptimisation::alternating_particle_scheme()
{
...
bool RobustOptimisation::best_directional_price_scheme()
{
...
bool RobustOptimisation::best_per_modality_scheme()
{
...
Coordinates RobustOptimisation::get_coordinates() {
    int *num_voxels;
    double *topleft;
    double *voxel_size;
...
std::vector<double> RobustOptimisation::calculate_modality_dose(std::vector<Aperture *> apertures) {
    std::vector<double> total_dose(this->total_voxels, 0.0);
    for (auto &aperture : apertures)
    {
        for (size_t i = 0; i < total_dose.size(); i++)
...
void RobustOptimisation::output_cost()
{
    for (auto &structure : this->structure_sets[0])
    {
...
void RobustOptimisation::export_modality_doses()
{
...
void RobustOptimisation::export_scenario_doses(bool interm)
{
    std::string buffer;
...
void RobustOptimisation::export_final_dose(bool interm)
{
    this->export_scenario_doses(interm);
    if (this->output_modalities) this->export_modality_doses();
}

void RobustOptimisation::write_dose(const std::string filename, const Coordinates coords, const std::vector<double> dose) {
...
void RobustOptimisation::export_final_weights(bool interm)
{
    std::string buffer;
...
void RobustOptimisation::export_data()
{
...
void RobustOptimisation::export_progress()
{
